...
const HistoricalEarthquakes = () => {
  const [yearRange, setYearRange] = useState([1897, 2026]);
  const [minMagnitude, setMinMagnitude] = useState(3);

  const filteredEarthquakes = useMemo(() => {
    return indiaEarthquakes.filter(eq => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      maxMagnitude: Math.max(...filteredEarthquakes.map(e => e.magnitude)),
      avgMagnitude: (filteredEarthquakes.reduce((s, e) => s + e.magnitude, 0) / filteredEarthquakes.length).toFixed(1),
      byRegion: filteredEarthquakes.reduce((acc, e) => {
        acc[e.region] = (acc[e.region] || 0) + 1;
        return acc;
      }, {} as Record<string, number>),
      byDecade: filteredEarthquakes.reduce((acc, e) => {
  ...
  const exportToJSON = () => {
    if (filteredEarthquakes.length === 0) return;
    const exportData = {
      metadata: { exportDate: new Date().toISOString(), yearRange: `${yearRange[0]}-${yearRange[1]}`, minMagnitude, totalRecords: filteredEarthquakes.length },
      earthquakes: filteredEarthquakes.map(eq => ({
        date: new Date(eq.time).toISOString().split("T")[0],
        magnitude: eq.magnitude, location: eq.location, state: eq.state, region: eq.region,
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">
            India Seismic History
          </h2>
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Explore India's notable earthquake catalog sourced from National Center for Seismology (NCS) &amp; USGS.
          </p>
        </div>
        ...
        <div className="glass-card rounded-2xl p-6 mb-8">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              ...
              <Slider value={yearRange} onValueChange={setYearRange} min={1897} max={2026} step={1} className="w-full" />
              <div className="flex justify-between text-xs text-muted-foreground mt-2"><span>1897</span><span>2026</span></div>
            </div>
            <div>
              <label className="block text-sm font-medium text-muted-foreground mb-3">
              ...
              <div className="flex justify-between text-xs text-muted-foreground mt-2"><span>M 2.0</span><span>M 8.0+</span></div>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6 mb-8">
            <div className="glass-card rounded-2xl p-6">
              <h3 className="font-semibold text-foreground mb-4 flex items-center gap-2">
                <BarChart3 className="w-5 h-5 text-primary" /> Earthquake Frequency by Decade
              </h3>
            ...
            <div className="glass-card rounded-2xl p-6">
              <h3 className="font-semibold text-foreground mb-4 flex items-center gap-2">
                <TrendingUp className="w-5 h-5 text-primary" /> Magnitude Distribution
              </h3>
              <div className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
        ...
        {stats && Object.keys(stats.byRegion).length > 0 && (
          <div className="glass-card rounded-2xl p-6 mb-8">
            <h3 className="font-semibold text-foreground mb-4 flex items-center gap-2">
              <MapPin className="w-5 h-5 text-primary" /> Earthquakes by Region
            </h3>
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {quake.isHistorical && (
                        <span className="inline-block mt-2 px-2 py-0.5 bg-primary/20 text-primary text-xs rounded-full">Historical Record</span>
                      )}
                    </div>
                  </div>
                </div>
        ...
        {filteredEarthquakes.length === 0 && (
          <div className="text-center py-12 glass-card rounded-2xl">
            ...
            <p className="text-muted-foreground">No earthquakes match the current filter criteria.</p>
          </div>
        )}
      </div>
    </section>
